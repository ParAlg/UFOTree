...
TEST(TernarizationSuite, constructor_test){
  ...
  TernarizedTree<TopologyTree<int, int>, int> tree2(5);
...
TEST(TernarizationSuite, test_determine_link_v){ 
  ...
  TernarizedTree<TopologyTree<int, int>, int> t(5);
  t.link(0, 1, 1);
  t.link(0, 2, 2);
  t.link(0, 3, 3);  
  ...
  TernarizedTree<TopologyTree<int, int>, int> t2(10);
  t2.link(0,1,1);
  t2.link(0,2,2);
  t2.link(0,3,3);
  t2.link(0,4,4); 
...
TEST(TernarizationSuite, test_link_cases_1){
  ...
  TernarizedTree<TopologyTree<int, int>, int> t(10);
...
TEST(TernarizationSuite, test_link_cases_2){
  ...
  TernarizedTree<TopologyTree<int, int>, int> t(20);
...
TEST(TernarizationSuite, test_random_maximal_star_graphs){
  ...
  for(int trial = 0; trial < num_trials; trial++){
    ...
    TernarizedTree<TopologyTree<int, int>, int> t(t_size);
...
TEST (TernarizationSuite, kary_tree_correctness_test) {
  vertex_t n = 256;
  vertex_t k = 10;
  TernarizedTree<TopologyTree<int, int>, int> tt(n);
