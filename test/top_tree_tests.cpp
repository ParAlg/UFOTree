...
TEST(Top_tree_suite, constructor_test){
    TopTree<int> t(3, PATH, [] (int a, int b){return std::max(a,b);}, std::numeric_limits<int>::max(), std::numeric_limits<int>::max());
    ASSERT_EQ(t.t.num_vertices, 3);
}

TEST(Top_tree_suite, basic_link_test){
    TopTree<int> t(10, PATH, [] (int a, int b){return std::max(a,b);}, std::numeric_limits<int>::max(), std::numeric_limits<int>::max());
