...
public:
  ...
  ParallelRCTreeTernarized(int _n, int _k = 1, std::function<aug_t(aug_t,aug_t)> _func = [] (int A, int B) {return std::max(A,B);}){
    n = _n * extra_tern_node_factor; 
    k = _k;
    tr = new ternarizer<int,int>{n, std::numeric_limits<int>::min()};
